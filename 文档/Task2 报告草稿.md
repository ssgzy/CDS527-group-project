...
> 最后更新：2026-04-04
> 状态：初稿完成，已对齐作业 rubric（2026-04-04）
...
**Data Types and Formats.** The data Google manages spans all three structural categories. Structured data includes search query logs, advertisement click records, and GPS coordinates from Maps — all of which are stored in well-defined schemas amenable to direct statistical analysis. Semi-structured data encompasses web pages (HTML), email messages (Gmail), and user preference files, which carry implicit structure but require parsing before analysis. Unstructured data — including images, videos, natural language text, and audio recordings from voice interfaces such as Google Home — constitutes the largest and fastest-growing share of Google's data estate, and presents the greatest processing complexity. Effectively handling this full spectrum of data formats is a prerequisite for any Big Data solution Google deploys.
...
**Distributed Computing Infrastructure and Analytics Methods.** To process and serve data at the velocity and volume the internet demands, Google developed a suite of proprietary distributed systems. The Google File System (GFS; Ghemawat et al., 2003) provides fault-tolerant storage across thousands of commodity servers, enabling the management of petabyte-scale data without relying on expensive specialised hardware. MapReduce (Dean & Ghemawat, 2004) allows computationally intensive tasks — such as building or updating the search index — to be parallelised across thousands of machines, dramatically reducing processing time. Sitting atop this infrastructure, Google applies a range of Big Data analytics techniques to generate insight. Statistical analysis, including A/B testing, is used extensively to evaluate competing search ranking configurations and advertisement layouts, enabling data-driven optimisation of user experience and revenue. Semantic and text analysis — encompassing natural language processing (NLP) — underpins both search intent understanding and features such as Gmail's Smart Reply. Machine learning clustering algorithms segment users into behavioural groups to power personalised recommendations and targeted advertising. Visual analytics, such as geographic heat maps derived from Maps usage data, further support both product development and advertiser targeting decisions.
